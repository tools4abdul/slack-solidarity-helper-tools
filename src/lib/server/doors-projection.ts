...
import { CAMPAIGN_TIME_ZONE } from '../campaign-time.js';
//
 ...
 *  door-knocking hours (8 am – 9 pm campaign-local) rather than assuming 24/7
...
// Door-knocking hours: 8 am – 9 pm in CAMPAIGN_TIME_ZONE.
...
function campaignMsOfDay(ms: number): number {
...
/** Milliseconds of door-knocking time (8 am – 9 pm campaign-local) between
...
export function knockableMsBetween(startMs: number, endMs: number): number {
	...
	for (let i = 0; cursor < endMs && i < 3_000; i++) {
		const msOfDay = campaignMsOfDay(cursor);
