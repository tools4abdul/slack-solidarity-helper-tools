...
**Reading the campaign's entries back.** Every sync, a packet whose Status is `Out` or `Complete`, or `Unwalked` with a canvasser named, and whose Canvasser is not one the app wrote, marks that turf as assigned — hidden from volunteers and refused on claim, exactly like turf an organizer handed out in VAN. `Incomplete` does not block: that turf is back in play. The claim itself also reads the tracker live, so a packet written down since the last sync is still caught; if Google is slow or unreachable the claim falls back to what the last sync saw rather than refusing everyone.
